...
# Perbaikan: grouping \b(?: ... )\b
KEYWORD_EMOJI_MAP = {
    r"\b(?:shrimp|prawn|udang)\b": "🦐",
    r"\b(?:cow|sapi|moo+)\b": "🐮",
    r"\b(?:cat|kucing)\b": "🐱",
    r"\b(?:dog|anjing)\b": "🐶",
    r"\b(?:fish|ikan)\b": "🐟",
    r"\b(?:rocket|to the moon)\b": "🚀",
    r"\b(?:fire|panas|hot)\b": "🔥",
    r"\b(?:heart|love|cinta)\b": "❤️",
    r"\b(?:star|bintang)\b": "⭐",
    r"\b(?:money|duit|uang|cash)\b": "💸",
    r"\b(?:coffee|kopi)\b": "☕",
    r"\b(?:tea|teh)\b": "🍵",
    r"\b(?:idea|ide)\b": "💡",
    r"\b(?:brain|otak)\b": "🧠",
    r"\b(?:muscle|gym|angkat besi)\b": "💪",
    r"\bthumbs?\s*up\b": "👍",
    r"\b(?:party|pesta|selametan)\b": "🎉",
    r"\b(?:laugh|lol|wkwk+)\b": "😂",
    r"\b(?:sad|sedih)\b": "😢",
}
...
def load_api_keys(provider_name: str, txt_filename: str, env_multi: str, env_single: str):
    """
    Urutan:
      1) Baca file TXT bila ada (SATU KEY PER BARIS; baris kosong / diawali '#' di-skip)
      2) Kalau tidak ada file/isi kosong: baca ENV multi (HANYA dipisah ENTER/newline)
      3) Kalau masih kosong: fallback ENV single
    TIDAK ada parsing koma.
    """
    keys = []
    ...
    try:
        ...
        log_message(f"Warning reading {txt_filename}: {e}", "WARNING")

    # 2) ENV multiline (splitlines)
    if not keys:
    ...
    if not keys:
        log_message(f"No API keys found for {provider_name}", "WARNING")
    else:
        log_message(f"{provider_name}: loaded {len(keys)} key(s): " +
                    ", ".join(_mask_key(k) for k in keys), "INFO")
    return keys


# ================== PROVIDER CALLERS (ROTASI KEY) ==================
def try_openai(system_prompt: str, user_prompt: str, retry_count: int):
    url = "https://api.openai.com/v1/chat/completions"
    models_to_try = ["gpt-4o-mini", "gpt-4.1-mini", "gpt-4o"]
    model_name = models_to_try[retry_count % len(models_to_try)]
    keys = load_api_keys("OpenAI", "openai_keys.txt", "OPENAI_KEYS", "OPENAI_API_KEY")
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        try:
            log_message(f"[OpenAI] try key {idx}/{len(keys)} {_mask_key(key)}", "INFO")
            ...
            if resp.status_code in (401, 403, 429):
                try:
                    ...
                    log_message(f"[OpenAI] switch key due to {resp.status_code}: {err}", "WARNING")
                except Exception:
                    log_message(f"[OpenAI] switch key due to {resp.status_code}", "WARNING")
                continue
            if 500 <= resp.status_code < 600:
                log_message(f"[OpenAI] server error {resp.status_code}, switching key", "WARNING")
                continue
            log_message(f"[OpenAI] non-OK {resp.status_code}: {resp.text[:200]}", "WARNING")
        except requests.exceptions.RequestException as e:
            log_message(f"[OpenAI] network {e} — switch key", "WARNING")
            continue
    return None


def try_gemini(system_prompt: str, user_prompt: str, retry_count: int):
    composed = (
    ...
    model_name = models_to_try[retry_count % len(models_to_try)]
    keys = load_api_keys("Gemini", "gemini_keys.txt", "GEMINI_KEYS", "GEMINI_API_KEY")
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        try:
            log_message(f"[Gemini] try key {idx}/{len(keys)} {_mask_key(key)}", "INFO")
            ...
            if resp.status_code in (401, 403, 429):
                log_message(f"[Gemini] switch key due to {resp.status_code}: {resp.text[:160]}", "WARNING")
                continue
            if 500 <= resp.status_code < 600:
                log_message(f"[Gemini] server error {resp.status_code}, switching key", "WARNING")
                continue
            log_message(f"[Gemini] non-OK {resp.status_code}: {resp.text[:200]}", "WARNING")
        except requests.exceptions.RequestException as e:
            log_message(f"[Gemini] network {e} — switch key", "WARNING")
            continue
    return None


def try_openrouter(system_prompt: str, user_prompt: str):
    url = "https://openrouter.ai/api/v1/chat/completions"
    keys = load_api_keys("OpenRouter", "openrouter_keys.txt", "OPENROUTER_KEYS", "OPENROUTER_API_KEY")
    ...
    for idx, key in enumerate(keys, start=1):
        ...
        try:
            log_message(f"[OpenRouter] try key {idx}/{len(keys)} {_mask_key(key)}", "INFO")
            resp = requests.post(url, headers=headers, json=payload, timeout=15)
            if resp.status_code == 200:
                data = resp.json()
                return (data.get("choices", [{}])[0]
            ...
            if resp.status_code in (401, 402, 403, 429):
                log_message(f"[OpenRouter] switch key due to {resp.status_code}: {resp.text[:160]}", "WARNING")
                continue
            if 500 <= resp.status_code < 600:
                log_message(f"[OpenRouter] server error {resp.status_code}, switching key", "WARNING")
                continue
            log_message(f"[OpenRouter] non-OK {resp.status_code}: {resp.text[:200]}", "WARNING")
        except requests.exceptions.RequestException as e:
            log_message(f"[OpenRouter] network {e} — switch key", "WARNING")
            continue
    return None
...
def generate_ai_response(user_message, display_name, retry_count=0):
    """
    Master generator: rotasi key per provider (ENTER-separated list),
    dan fallback antar provider bila semua key provider tsb habis.
    Urutan provider bisa diubah di provider_order.
    """
    ...
    for prov in provider_order:
        if prov == "openai":
            text = try_openai(system_prompt, user_prompt, retry_count)
        elif prov == "gemini":
            text = try_gemini(system_prompt, user_prompt, retry_count)
        elif prov == "openrouter":
            text = try_openrouter(system_prompt, user_prompt)
        if text:
            final, added_emoji = _finalize_ai_text(text, display_name, user_message)
            return final, added_emoji

    log_message("All providers/keys failed", "ERROR")
    return None, False
...
def get_recent_messages(channel_id, headers, limit=50):
    try:
        ...
        if response.status_code == 200:
            ...
            try:
                retry_after = float(response.headers.get("retry-after") or response.json().get("retry_after", 5))
            except Exception:
                retry_after = 5
            log_message(f"Discord rate limit hit - waiting {retry_after}s", "WARNING")
...
def send_message(channel_id, content, headers, reply_to_message_id=None, retry_count=0):
    ...
    try:
        ...
        if not content:
            log_message("Skip send: empty content", "WARNING")
            return False
        if len(content) > 2000:
        ...
        if reply_to_message_id:
            payload["message_reference"] = {"message_id": reply_to_message_id, "fail_if_not_exists": False}
        ...
        if response.status_code in (200, 201):
            ...
            if retry_count < max_retries:
                try:
                    wait_time = float(response.headers.get("retry-after") or response.json().get("retry_after", 5))
                except Exception:
                    wait_time = 5
            ...
            try:
                ...
                info = {"message": response.text[:200]}
            log_message(f"Failed to send (400): {info}", "ERROR")
            return False
        else:
            txt = response.text[:200]
            log_message(f"Failed to send message: {response.status_code} - {txt}", "ERROR")
            return False
    except requests.exceptions.RequestException as e:
...
def worker_main():
    ...
    try:
        ...
        process_count = int(
            ...
            or ("50" if non_interactive or HEADLESS
                else input(Fore.CYAN + "How many recent messages to scan each check (1–100): ").strip())
        )
        ...
        reply_chance = float(
            ...
            or ("0.25" if non_interactive or HEADLESS
                else input(Fore.CYAN + "Reply chance for normal messages (0–1): ").strip())
        )
        ...
        thread_reply_chance_in = os.getenv("THREAD_REPLY_CHANCE") or (
            ...
            else input(Fore.CYAN + "Reply chance for replies in threads to others (0–1) [0.35]: ").strip()
        )
        ...
        allow_time_greet = (
            ...
            or ("n" if (non_interactive or HEADLESS)
                else input(Fore.CYAN + "Allow time-based greetings? (y/n) [n]: ").strip())
        ).lower()
        ...
        MAX_THREAD_REPLIES = int(
            ...
            or ("3" if (non_interactive or HEADLESS)
                else input(Fore.CYAN + "Max replies per thread (1–5) [3]: ").strip() or "3")
        )
        MAX_THREAD_REPLIES = max(1, min(5, MAX_THREAD_REPLIES))
        FOLLOWUP_CONTINUE_PROB = float(
            os.getenv("FOLLOWUP_CONTINUE_PROB")
            or ("0.60" if (non_interactive or HEADLESS)
                else input(Fore.CYAN + "Chance to continue follow-up (0–1) [0.60]: ").strip() or "0.60")
        )
        ...
        MIN_REPLY_DELAY = int(
            ...
            or ("3" if (non_interactive or HEADLESS)
                else input(Fore.CYAN + "Min typing delay before send (sec) [3]: ").strip() or "3")
        )
        MAX_REPLY_DELAY = int(
            os.getenv("MAX_REPLY_DELAY")
            or ("10" if (non_interactive or HEADLESS)
                else input(Fore.CYAN + "Max typing delay before send (sec) [10]: ").strip() or "10")
        )
        ...
        min_delay = int(
            os.getenv("MIN_DELAY")
            or ("45" if (non_interactive or HEADLESS)
                else input(Fore.CYAN + "Minimum delay between checks (seconds): ").strip())
        )
        max_delay = int(
            os.getenv("MAX_DELAY")
            or ("90" if (non_interactive or HEADLESS)
                else input(Fore.CYAN + "Maximum delay between checks (seconds): ").strip())
        )
